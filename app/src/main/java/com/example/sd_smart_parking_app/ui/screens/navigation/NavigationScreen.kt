...
import com.example.sd_smart_parking_app.ui.components.CompassArrow
...
import com.example.sd_smart_parking_app.viewmodel.NavigationViewModel
import com.example.sd_smart_parking_app.data.MapsHelper
        ...
        Column(
            ...
            Card(
                ...
                Row(
                    ...
                    InfoColumn(
                        icon = "🛣️",
                        value = String.format("%.1f", navigationState.distanceToDestination / 1000),
                        label = "kilometers"
                    )
            ...
            // Componente de brújula rotativa
            if (navigationState.isCompassAvailable) {
                Box(
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(Spacing.lg),
            ...
            // Botón Iniciar Viaje
            if (navigationState.isLocationAvailable && navigationState.currentLocation != null) {
                PrimaryButton(
                    text = "Iniciar Viaje",
                    onClick = {
                        navigationState.currentLocation?.let { location ->
                            MapsHelper.openMapsNavigation(
                                context,
                                location.latitude,
                                location.longitude
                            )
            ...
            // Estado de ubicación
            if (navigationState.isLocationAvailable) {
                Box(
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(horizontal = Spacing.lg, vertical = Spacing.md)
                        .background(
                            color = com.example.sd_smart_parking_app.ui.theme.HighAvailabilityGreen.copy(alpha = 0.1f),
                            shape = RoundedCornerShape(CornerRadius.md)
                        )
                        .padding(Spacing.md)
                ) {
                    Text(
                        text = "📍 Location: ${navigationState.currentLocation?.latitude?.let { String.format("%.4f", it) }}, ${navigationState.currentLocation?.longitude?.let { String.format("%.4f", it) }}",
                        style = Typography.bodySmall
                    )
                }
            }
